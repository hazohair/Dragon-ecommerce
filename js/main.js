...
if(localStorage.getItem("app_version") !== null){
    ...
    if(localStorage.getItem("app_version") === "english"){
        ...
        shopSneakersBtn.innerHTML = "Shop Sneakers";
        shopSneakersBtn.removeAttribute("style")
        swiper_header.innerHTML = "Top New Products";
        swiper_header.removeAttribute("style");
        ...
        shopSneakersBtn.innerHTML = "تسوق احذية سنيكرز";
        shopSneakersBtn.style.fontSize = "1.5rem";
        swiper_header.innerHTML = "أحدث المنتجات";
        swiper_header.style.fontSize = "1.5rem";
        swiper_header.style.paddingRight = "5%";
...
languageBox.addEventListener('change',function(event){
...
function createShoppingCart(){
    ...
    */
    ...
    document.querySelector(".bg-container").addEventListener('click',function(){
...
if(JSON.parse(localStorage.getItem("orderDeatils")).length == 0 ){
    ...
    document.querySelector(".bg-container").after(cartMainContainer);
}else{
    ourCartShop.remove();
}
}
